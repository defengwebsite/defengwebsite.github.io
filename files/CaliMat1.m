function [X,z_e,z_l,z_u] = CaliMat1(G,e,I_e,J_e,l,I_l,J_l,u,I_u,J_u,OPTIONS)
...
tol_bicg       = 1.0e-2;   % termination for BiCGStab
tau0           = 1.0e-8;   % lower bound of minimum eigenvalue
maxit          = 200; 
maxit_bicg     = 200;      % maximum iterations in BiCGStab
maxit_step     = 20;       % maximum iterations in line search
delta          = 0.5;      % decrease rate in step length
use_precond    = 1;        % use preconditioner
name_smoothfun = 'Huber';  % use Huber smoothing function
%name_smoothfun = 'Smale';  
...
r_const0  = r;
sigma     = 0.5e-6;                 % decrease in norm of the merit function  
...
if exist('OPTIONS')  
    ...
    if isfield(OPTIONS,'tol_bicg');       tol_bicg        = OPTIONS.tol_bicg; end
    if isfield(OPTIONS,'maxit');          maxit           = OPTIONS.maxit; end
    if isfield(OPTIONS,'maxit_bicg');     maxit_bicg      = OPTIONS.maxit_bicg; end
    if isfield(OPTIONS,'maxit_step');     maxit_step      = OPTIONS.maxit_step; end    
...
[phi,g_ze,g_zl,g_zu,fe,fx] = Phi(name_smoothfun,epsilon,e,z_e,I_e,J_e,...
    l,z_l,I_l,J_l,u,z_u,I_u,J_u,P,X,lambda,kappa,c);  % the initial norm of the merit function
...
tt = etime(clock,t0);
[hh,mm,ss] = time(tt);
fprintf('\n   Iter.  No.of BiCGs  Step Length    Smoothing Par.  Norm of SmFunc.   time_used ')
fprintf('\n    %d      %2.0d               %3.2e               %3.2e         %3.2e        %d:%d:%d ',0,str2num('-'),str2num('-'),epsilon,phi^0.5,hh,mm,ss)
...
while (phi^0.5>tol && k1<maxit)
    ...
    if k_l+k_u == 0       % if no ineqalities, use CGs; otherwise bicgstab
        [d_z, flag, relres, iterk] = pre_cg(b_z,name_smoothfun,epsilon,e,I_e,J_e,...
            l,I_l,J_l,u,I_u,J_u,fx,Omega,P,maxit_bicg,tol_bicg,kappa,phi,eta,c,prec);
        if k1==0
    ...
    [phi,g_ze,g_zl,g_zu,fe,fx] = ...
        Phi(name_smoothfun,epsilon,e,z_e,I_e,J_e,l,z_l,I_l,J_l,u,z_u,I_u,J_u,P,X,lambda,kappa,c); 
    phi_time = phi_time + etime(clock,t1); 
    f_eval = f_eval + 1;
    
    % Line Search 
    k_inner=0;
    while( k_inner<=maxit_step && phi>(1-2*sigma*(1-sqrt(2)*max(eta,r*eps_bar))*delta^k_inner)*phi0 + 1.0e-12 )       
        k_inner = k_inner+1;
    ...
    tt = etime(clock,t0);
    [hh,mm,ss] = time(tt); 
    fprintf('\n   %2.0d        %2.0d          %3.2e       %3.2e         %3.2e        %d:%d:%d ',k1,iterk,delta^k_inner,epsilon,phi^0.5,hh,mm,ss)
...
r_X = length(find(s>0));              % rank of X*
r_Z = length(find(abs(s-lambda)>0));  % rank of Z
lambda_min= min(abs(lambda))

% optimal primal and negative dual value
P = (X-G);
P = (P + P')/2;
prim_val = sum((sum(P.*P)))/2;
...
fprintf(' Computing time for the merit function = %3.1f \n', phi_time);
...
switch name_smoothfun
    case 'Huber'
        ...
        if (r.b<n&&r.s<n)  %if r.b==n, Omega.bb=ones(n,n); if r.b==n, Omega.ss=zeros(n,n).
            db = lambda(1:r.b);
